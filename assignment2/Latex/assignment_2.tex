...
When considering the problem of mean independence (A3), there can be multiple root causes to this issue; ommitted variable bias, endogegenous treatment, sampling bias, attrition bais, simultaneous caulsailty bias. All of these issues introduce backdoor pathways \textbf{CITE ECI HERE}, which bias the (eg. OLS) estimator severely. 
...
%One example of such conditions may be a situation, where the preference of students for
%receiving more years of education can be associated with the level of wealth of the close family,
%e.g. parents, which motivate their children learn longer and fund their higher education. Also, it
%can be arguably assumed that wealthier families have better access to good-paying jobs in some
%sectors (f.ex. business, medicine, law) and can help their children find a better-paying job (as
%many jobs are found through network of professionals). At In other words, people from those
%families would receive higher salaries regardless of their level of education. In this case, the not
%including the economic background of the family can create an omitted variable bias, because
%the wealth of the family (being the part of ϵ is supposedly associated with the effect of education
%on wages.
%The second example of a bias can be the quality of education received, which in the USA
%is, in many cases, correlated with the average income in the neighbourhood on the primary and
%secondary level (districts/counties with lower incomes have higher education budgets, which is
%possibly associated with the quality of education). As there is no information in the data about
%the mean/median income in the area of residence or the quality of education receive, it may be
%arguably assummed that people leave at poorer areas receive worse education and are more likely
%to drop out. At the same time, the wages in poorer areas are lower than in wealthy ones, so it
%states another omitted variable bias.
...
\textbf{We can also do the partial F test here. But we do it manually iwth an OLS}
\begin{enumerate}
