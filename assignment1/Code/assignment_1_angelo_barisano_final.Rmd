...
```{r }
A4
# Tests andcontrols for Homoscedasitcity
...
# library("AER")
# vif <- vif(multi_var_model_w_confounders_ninteraction, type = 'predictor')
# stargazer(vif, type = 'text')
vif_ <- vif(multi_var_model_w_confounders_ninteraction)
stargazer(vif_, type = 'text')

#' while there are some variables that dispaly a VIF of more than 5, this does not automatically indicate mulitcollineariy. however this necessitats further analysis of this issue to reduce the probabiltiy of having a problem with multicllinearity. Overall, the increased VIF can mostly be attributed to the incusion of interaction effects. 
...
```{r }
# A 6
df_n <- df_ %>% mutate(fitted = multi_var_model_w_confounders_ninteraction$fitted.values,
...
p2 <- qplot(sample = multi_var_model_w_confounders_ninteraction$residuals) + theme_classic()
...
df_n <- df_ %>% mutate(fitted = multi_var_model_w_confounders_ninteraction_unscaled$fitted.values,
                                     resids = multi_var_model_w_confounders_ninteraction_unscaled$residuals)
p1 <- ggplot(df_n, aes(x = resids)) +
  geom_histogram(color = "black", fill = "white") + theme_classic() +
  labs(x = "Residuals", y = "Frequency")
p2 <- qplot(sample = multi_var_model_w_confounders_ninteraction_unscaled$residuals) + theme_classic()
grid.arrange(p1, p2, nrow = 1)
