...
multi_var_model_w_confounders_ninteraction <-
  lm(
    ln_SalePrice ~ 1 + tot_liv_area  + y_since_rem + low_density_zone +  commercial_zone + floating_zone + I(tot_liv_area ^2) + tot_liv_area * low_density_zone + tot_liv_area * commercial_zone + tot_liv_area * floating_zone + LotArea + YrSold  + OverallQual_cat + Building_type + tot_liv_area * LotArea ,
    data = df_
  )
...
multi_var_model_w_confounders_ninteraction_unscaled <-
  lm(
    SalePrice ~ 1 + tot_liv_area  + y_since_rem + low_density_zone +  commercial_zone + floating_zone + tot_liv_area * low_density_zone + tot_liv_area * commercial_zone + tot_liv_area * floating_zone + LotArea + YrSold  + OverallQual_cat + Building_type + LotArea*tot_liv_area ,
    data = df_
  )
...
```{r }
# stargazer(multi_var_model_w_confounders,
# multi_var_model_w_confounders_ninteraction_unscaled,
# multi_var_model_w_confounders_ninteraction, type = 'text')
...
stargazer(
  ...
  p = list (
    ...
    coef(summary(multi_var_model_w_confounders_ninteraction_unscaled))[, 4],
    coef(summary(multi_var_model_w_confounders_ninteraction_unscaled.beta))[, 5],
...
```{r A4}
# Tests and controls for Homoscedasitcity
# Run models with clustered SE, robust SE, and normal se
# multi_var_model_w_confounders_ninteraction
# 
# multi_var_model_w_confounders_ninteraction_unscaled
...
# Make plots
# png(filename="normalityplots.png")
# par(mfrow = c(2,2))
# plot(multi_var_model_w_confounders_ninteraction)
# dev.off()
...
# robust SE
# seBasic <- sqrt( diag( vcov( m_2 )))
# seWhite <- sqrt( diag(vcovHC( m_2, type="HC0")))
# stargazer(m_2,m_2,se=list(seBasic,seWhite),
# omit=c("LotArea","X2ndFlrSF","MSZoning","FullBath",
# "HalfBath","BedroomAbvGr","BasementDummy",
# "PoolDummy"),no.space=TRUE,font.size="footnotesize",
# column.labels = c("Regular standard error","Robust standard error"))
...
stargazer(multi_var_model_w_confounders_ninteraction, multi_var_model_w_confounders_ninteraction, multi_var_model_w_confounders_ninteraction, se = list (seBasic_full, seWhite_full, seClust_full), type = 'text' , omit = c(
    "YrSold2008",
    "YrSold2009",
    "YrSold2010",
    "OverallQual_cat3",
...
#' how to interpret?
#' eg income we see no real heteroscedasticity; so there is less varaince or SE 
#' but lunch was heteroscedastic --> the result ofthe fix is wider SE
...
vif <- corvif(multi_var_model_w_confounders_ninteraction, type = 'predictor')
stargazer(vif, type = 'text')
vif <- corvif(multi_var_model_w_confounders_ninteraction)
stargazer(vif, type = 'text')

#' while there are some variables that dispaly a VIF of more than 5, this does not automatically indicate mulitcollineariy. however this necessitats further analysis of this issue to reduce the probabiltiy of having a problem with multicllinearity. 
...
stargazer(vif, type = 'text')
#' if vif is higher than 5, it is bad!!!



# remedies: DO NOT TAKE OUT THE MULTICOLLINEAR VARIABLES!!!!
# better solution
# combine multicollinear varaibles (principal components); pretty much same explanation

# eg instead pf students and teachers you use te ratio size whcih is students to teachers
# so better combine 
